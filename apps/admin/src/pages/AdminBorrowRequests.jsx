  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <p>No pending borrow requests. New requests appear here when customers use “Request to borrow” on a book.</p>
      ) : (
          ...
          {requests.map((r) => (
            ...
            >
              <div style={{ flex: '1 1 280px' }}>
                ...
                {(r.extra_copy_reason) && (
                  <span style={{ display: 'inline-block', marginLeft: 8, padding: '2px 8px', background: '#dbeafe', color: '#1e40af', borderRadius: 6, fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Additional copy request</span>
                )}
                ...
                {(r.extra_copy_reason) && (
                  <div style={{ fontSize: 14, marginTop: 8, padding: 10, background: '#f0f9ff', borderRadius: 8, border: '1px solid #bae6fd' }}>
                    <strong style={{ color: '#0c4a6e' }}>Reason for additional copy</strong>
                    <div style={{ marginTop: 4 }}>{r.extra_copy_reason}{r.extra_copy_reason_custom ? ` — ${r.extra_copy_reason_custom}` : ''}</div>
                  </div>
                )}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {user?.role === 'super_admin' && !r.extra_copy_reason && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                    ...
                    />
                    Override 1-copy limit
                  </label>
                )}
                <button
                  type="button"
                  onClick={() => handleApprove(r.id, overrideCopy[r.id] || !!r.extra_copy_reason)}
