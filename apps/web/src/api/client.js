...
/** Try health against one URL; timeout 8s */
...
export const api = {
  ...
  books: {
    ...
    getSameGenre: (id, limit = 6) => request(`/books/${id}/same-genre?limit=${limit}`),
  ...
  borrow: {
    borrow: (bookId, opts = {}) => request('/borrow', { method: 'POST', body: JSON.stringify({ bookId, ...opts }) }),
    return: (payload) => request('/return', { method: 'POST', body: JSON.stringify(payload) }),
    myBooks: () => request('/my-books'),
    dueAlerts: () => request('/due-alerts'),
    reserve: (bookId) => request('/reserve', { method: 'POST', body: JSON.stringify({ bookId }) }),
    myReservations: () => request('/reservations'),
  ...
  admin: {
    ...
    verifyReturn: (id) => request(`/admin/returns/${id}/verify`, { method: 'POST' }),
